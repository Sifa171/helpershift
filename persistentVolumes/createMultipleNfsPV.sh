...
if [ -z "$AMOUNT" ]
then
    echo "Please provide a number of PersistentVolumes you want to create"
    exit 0
fi

while [ $i -le $AMOUNT ]
do
        ansible -i $INVENTORY $GROUP -m file -a "path=$NFS_ANSIBLE_DEST$i state=directory owner=nfsnobody group=nfsnobody mode=0777"
        oc process -f pv-template.yml -p NAME=$PV_NAME$i -p NFS_HOST=$NFS -p NFS_PATH=$NFS_ANSIBLE_DEST -p RECLAIM_POLICY=$POLICY -p STORAGE=$STORAGE -p ACCESS_MODE=$PV_ACCESS_MODE | oc create -f -
        i=$[$i+1]
done